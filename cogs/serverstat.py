...
class Statics(commands.Cog):
  ...
  async def userinfo(self, ctx, *, user: discord.Member = None):
    ...
    if today.strftime(age_format2) == user.created_at.strftime(
        age_format2) or today.strftime(age_format2) > user.created_at.strftime(
          age_format2):
...
#botinfo
  ...
  async def botinfo(self, ctx):
    user = ctx.author
    embed = discord.Embed(color=0xdfa3ff, title="mr Monkey's Profile:")
    embed.set_author(name=f"Requested by: {str(user)}",
                     icon_url=user.avatar.url)
    embed.add_field(name="Developer:", value="Nguyen Duy An (leby)")
    embed.add_field(
      name="Information:",
      value=
      "Feature-rich bot for entertainment and study, with more than 3000 lines of code!"
    )
    embed.add_field(name=chr(173), value=chr(173))
    embed.add_field(name="Prefix:", value="``!``")
    embed.add_field(name="➕ Add me:",
                    value="[click here](https://bit.ly/2ZEUiwo)")
    embed.add_field(name="👨‍💻 My owner:",
                    value="[click here](https://lebyleafy.github.io/profile/)")
    embed.set_footer(text="discord: .leby")
